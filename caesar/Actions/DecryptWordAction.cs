...
public class DecryptWordAction : ApplicationActionBase
{
    public DecryptWordAction(string actionName) : base(actionName)
    {
    }
    ...
    {
        base.PerformAction();
        var word = GetInput("Enter word to decrypt: ");
        try
        {
            var decryptedWord = Decrypt(word);
            PrintResult($"Caesar's encryption method returned: {decryptedWord}");
